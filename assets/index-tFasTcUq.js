...
width: 100%;
        ...
        `.trim(),document.head.appendChild(t)}enableEvents(e){return this.ctlElem===null?!1:(this.htmlMaker.enableTableEvents(this.ctlElem.className),this.onSelect=e,!0)}disableEvents(){return this.ctlElem===null?!1:(this.htmlMaker.disableTableEvents(this.ctlElem.className),!0)}setVisible(e){return this.ctlElem===null?!1:(this.ctlElem.hidden=!e,!0)}dispose(){super.dispose(),this.ctlElem!==null&&(this.ctlElem.parentElement?.removeChild(this.ctlElem),this.ctlElem.remove()),this.disableEvents(),this.ctlElem=null}tblCssName(){return`${this.ctlName}-tbl`}}class _t{constructor(){this.key="",this.text=""}}class Zt extends at{constructor(){super(...arguments),this.callerName="",this.htmlMaker=new L,this.table=new K,this.ctlElem=null,this.headerElem=null,this.footerElem=null}setGridtems(e){this.items=e}setListener(e,t,s,i="",n=""){if(this.items===void 0)return!1;this.dispose(),this.callerName=s,this.ctlName=e;const o=this.items.length,l=new K;l.makeDim(2,o);let a=0;for(const B of this.items)l.getCell(0,a).typeInfo.setLabel(`${B.text}`,!1),l.getCell(0,a).className=this.keyCssName(),l.getCell(0,a).typeInfo.using.itemId=a,l.getCell(1,a).typeInfo=B.value.items[0].typeInfo,l.getCell(1,a).className=this.valueCssName(),l.getCell(1,a).typeInfo.using.itemId=a,a++;const r=this.firstAction(e,t);if(r===null)return!1;const c=l.ToScrollHTML(e,this.tblCssName()),h=new V;h.props.name="",h.props.id=this.tblCssName(),h.props.className=this.tblCssName(),h.props.option.setTable(c),h.props.option.onSelect=async B=>{if(this.onSelect!==void 0){const A=new Ie;A.callerName=B.classify,A.result=B.selectedValue,await this.onSelect(A)}},this.htmlMaker=new L,this.htmlMaker.add(h);const u=this.htmlMaker.ToHTML();let d=document.createElement("dialog");d.className=e,d.innerHTML=u,this.ctlElem=d;let p=d.children[0];const $=this.divCssName();p.className=$,p.id=$;let k=p.children[0];return k.className=`${this.tblCssName()}`,k.id=`${this.tblCssName()}`,i!==""&&(this.headerElem=document.createElement("div"),this.headerElem.className=i,this.headerElem.id=i,d.appendChild(this.headerElem)),r.appendChild(this.ctlElem),n!==""&&(this.footerElem=document.createElement("div"),this.footerElem.className=n,this.footerElem.id=n,d.appendChild(this.footerElem)),this.table=l,!0}show(e,t){this.ctlElem!==null&&(this.ctlElem.style.left=`${e}px`,this.ctlElem.style.top=`${t}px`,this.ctlElem.show())}setFontConfig(e="0.9"){this.table.setFontConfig(e)}applyCss(){const e=`${this.getBaseElem().className}-flgrd-style`;if(document.getElementById(e))return;const t=document.createElement("style");t.id=e,t.textContent=`
${this.getOuterCssText()}
...
`.trim(),document.head.appendChild(l);const a=document.getElementById(this.parentName);this.htmlMaker.copyCssToInlineStyle(`.${this.parentName}`,a)}}const we={Unknown:"Unknown",Success:"Success",Fail:"Fail",Cancel:"Cancel"};async function Tt(m){const e=m.isWebRunning,t=m.currentUserHome,s=m.chStatusListFile,i=m.chListFile,n=new Kt,o=new L,l="splash-screen";o.initFullScreen(l,"エボナ データベース初期化中..."),o.applyFullScreenCss(l);const a=o.getFullScreenMsgElem(l),r=a!==null?a.innerHTML:"";if(t===m.user1Home){const f=[{ns:H.CnsRed,nsName:"赤属性"},{ns:H.CnsBlue,nsName:"青属性"},{ns:H.CnsGreen,nsName:"緑属性"},{ns:H.CnsYellow,nsName:"黄属性"},{ns:H.CnsViolet,nsName:"紫属性"}],M=await new Te().LoadList(i);for(const C of f)a!==null&&(a.innerHTML=`${r} ${C.nsName}`),await n.setupNs(C.ns,m,M)}if(t===m.user2Home){const f=[{ns:H.CnsBlue,nsName:"藍属性"},{ns:H.CnsRed,nsName:"紅属性"},{ns:H.CnsGreen,nsName:"翠属性"},{ns:H.CnsYellow,nsName:"黄属性"},{ns:H.CnsWhite,nsName:"天属性"},{ns:H.CnsBlack,nsName:"冥属性"}],M=await new Te().LoadList(i);for(const C of f)a!==null&&(a.innerHTML=`${r} ${C.nsName}`),await n.setupNs(C.ns,m,M)}a!==null&&(a.innerHTML="UI 初期化中 ...");const c=new Ge,h=new qe;await h.loadDB(s);const u=await h.loadAuxScoreSet(m.gameType);async function d(){async function f(){return await new Te().LoadList(i)}const g=await f();g.uiInfo.name="charListArea",g.uiInfo.left="300",g.uiInfo.top="100";const M=g.uiInfo.name,C=await g.toHTML(n);if(e){const y="キャラ選択",S=g.createSelectorBox(y,M,C);g.addEventHandlers(S),g.addItemEventHandlers(),g.enableLazyImages(n),S.show();const T=new X;T.setAsDlg(S,y),c.add(T)}return g}const p=await d();async function $(){const f=new rs;await f.load(),f.uiInfo.name="CharSummary",f.uiInfo.left="400",f.uiInfo.top="100";const g=f.uiInfo.name,M=await f.toHTML();if(e){const C="キャラ一覧",y=f.createSummaryBox(C,g,M);await f.updateCharInfos(n),f.addEventHandlers(y,async T=>{T.selectCh=p.selectedCh;const P=await n.getImageUrlBy(p.selectedCh.iconFileName,p.selectedCh.ns);if(P===null)T.selectedImg="",T.cancel=!0;else{T.selectedImg=P,T.cancel=!1;const R=await h.getStatus(p.selectedCh.idAsText);console.log(R),T.scoreSet=R,T.scoreSet.items=T.scoreSet.items.concat(u.items)}console.log(`selected ch = ${T.selectCh.name}`)}),f.addItemEventHandlers(),f.enableLazyImages(n),y.show();const S=new X;S.setAsDlg(y,C),c.add(S)}return f}const k=await $();async function B(f,g,M,C){const y=new hs;f===A&&(y.InitForPlayer(k),y.autoForm=!0,y.editFormEnable=!1,y.saveEnable=!1,y.imgLoader=n),f===J&&(y.InitForEnemy(h),y.SetAuxScoreSet(u),y.autoForm=!1,y.editFormEnable=!0,y.saveEnable=!0),y.formation.uiInfo.name=f,y.formation.uiInfo.left=`${g}`,y.formation.uiInfo.top=`${M}`;const S=y.formation.uiInfo.name,T=await y.toHTML(S),P=f===J?y.toGridHTML():"";if(e){const R=y.createFormationBox(C,S,T,P);y.addEventHandlers(R),y.addItemEventkHandlers(async E=>{let D="",We="";E.selectedImg===""?(D=p.selectedCh.iconFileName,We=p.selectedCh.ns,E.selectCh=p.selectedCh):(D=E.item.ch.iconFileName,We=E.item.ch.ns,E.selectCh=E.item.ch);const bt=await n.getImageUrlBy(D,We);bt!==null&&(E.selectedImg=bt,E.item.isEmpty=!1,Ct(E.uiName,E.item,E.selectCh))},async E=>{const D=await n.getImageUrlBy(E.selectedImg,p.selectedCh.ns);D!==null&&(E.selectedImg=D,E.item.isEmpty=!0,Ct(E.uiName,E.item,E.selectCh))},async E=>{console.log(`selected ch = ${E.item.ch.name}`);const D=new At;D.selectCh=E.item.ch,D.selectedImg=E.selectedImg,D.scoreSet=E.scoreSet,k.charStock(D)},async E=>{const D=yt();console.log(D),await D.replaceJudge(n)}),y.enableLazyImages(n),R.show();const ye=new X;ye.setAsDlg(R,C),c.add(ye)}return y}const A="playerForm",J="enemyForm",Ee=await B(A,100,100,"自編成"),xe=await B(J,100,200,"敵編成");async function Pt(f,g,M,C){q.uiInfo.name=f,q.uiInfo.left=`${g}`,q.uiInfo.top=`${M}`;const y=await q.toHTML("combatTable",n),S=q.createCombatBox(C,f,y);q.enableLazyImages(n),await q.replaceJudge(n),S.show();const T=new X;T.setAsDlg(S,C),c.add(T)}const q=yt();await Pt("combatForm",120,300,"対戦予想");const ht="保存";{const f=new X;f.setAsMenu(ht),c.add(f)}const mt="復帰";{const f=new X;f.setAsMenu(mt),c.add(f)}let Me=null;const Ot=await c.toHTML("dockForm",n);if(e){const f=c.createDockBox("dockForm",Ot);c.addItemClickHandlers(async g=>{c.stdApplyAction(g)!==!1&&g.item.isMenuType&&(g.item.toolTip===ht&&await Ut(),g.item.toolTip===mt&&await zt(async C=>{if(console.log(`[loadedResult] ${C}`),C!==we.Success)return;const y=Q.fromJsonInst(ve),S=Q.fromJsonInst(Le);ve=null,Le=null,await Ee.Setup(y,n),await xe.Setup(S,n)}))}),c.enableLazyImages(n),f.show(),Me=f}const dt="playerForm.json",ut="enemyForm.json",pt="dockForm.json";async function Ut(){c.InitZOrder(Z);const f=Q.toJsonText(Ee.formation,Ee.scsList),g=Q.toJsonText(xe.formation,xe.scsList),M=Ve.toJsonText(c),C=new window.JSZip;C.file(dt,f),C.file(ut,g),C.file(pt,M);const y=await C.generateAsync({type:"blob"}),S="gameConfig.zip",T=URL.createObjectURL(y),P=document.createElement("a");P.href=T,P.download=S,P.click(),URL.revokeObjectURL(T),console.log("saved!")}let ft=null,ve=null,Le=null;async function zt(f){const g=document.createElement("input");return g.type="file",g.accept=".zip",g.addEventListener("cancel",()=>(console.log("Cancelled."),we.Cancel)),g.addEventListener("change",async()=>{if(g.files.length==1){console.log("File selected: ",g.files[0].name);const C=await g.files[0].arrayBuffer(),S=await new window.JSZip().loadAsync(C);async function T(R){const ye=S.file(R);if(ye){const E=await ye.async("string"),D=JSON.parse(E);return console.log(D),D}}{const R=await T(pt);R&&(ft=R)}{const R=await T(dt);R&&(ve=R)}{const R=await T(ut);R&&(Le=R)}const P=ft!==null&&ve!==null&&Le!==null?we.Success:we.Fail;f(P)}}),g.click(),we.Unknown}const Z=new b;e&&(Z.AddDialogs(),Z.AssignIndexies(),await Z.LoadAllSetting(),await Z.loadSetting(Me),c.InitZOrder(Z),await Z.ForEachAsync(f=>{const g=j.FindDialogParent(f);return g!==null&&(m.isLocal||f==="charListArea"?g.hidden=!1:g.hidden=m.evonaType!==W.Full),!0}),Me!==null&&(Me.parentElement.hidden=!m.isLocal)),o.hideFullScreenCss(l);function gt(f){const g=new ct,M=new rt,C=f.formation;C.uiInfo.name,C.uiInfo.name,f.scsList===void 0&&console.log(`[${f.formation.uiInfo.name}] 対戦予想結果の作成は工事中です`);let y=0;for(const S of C.items){if(f.scsList===void 0)continue;const T=f.scsList[y];y++;const P=Math.ceil(T.stdScore),R=new Ht(S.ch.id,S.ch.name,S.isEmpty,P);R.allAvailable=T.allAvailable,M.Add(R)}return g.Add(M),g.debug(),g}function yt(){const f=new us,g=gt(Ee),M=gt(xe),C=new it;C.setScoreItems(g.groupRows[0].columns),C.boost=100;const y=new it;y.setScoreItems(M.groupRows[0].columns),y.boost=100;const S=new ds;S.setPlayer(C),S.setEnemy(y),f.setPair(It.HiLv,S),f.calcCombatScore();for(const[T,P]of f.combatPairs){const R=P.judge(G.None);console.log(`judge=[${R}]`)}return f}async function Ct(f,g,M){const C=It.HiLv;q.combatPairs.get(C),f===A&&await q.replaceChar(C,_.Player,g,M,n),f===J&&await q.replaceChar(C,_.Enemy,g,M,n),await q.replaceJudge(n)}}async function ps(m){if(!m.isWebRunning)return;const e=m.isWebRunning;console.log(`mode=${m.edit}`);const t=new Ge;async function s(){const u=re.Resource,d=new Nt;d.init(),await d.load(u,z.none),d.uiInfo.name="ResourceEdit",d.uiInfo.left="110",d.uiInfo.top="10";const p=d.uiInfo.name,$=await d.toHTML(d.makeResEditItems(),d.makeResLog);if(e){const k=document.createElement("div");k.id=d.uiInfo.name,k.className=d.uiInfo.name,document.body.appendChild(k);const B="文字列リソース",A=d.createEditorBox(B,p,$);d.addEventHandlers(A),d.addItemEventHandlers(),A.show(),d.enableResize();const J=new X;J.setAsDlg(A,B),t.add(J)}return d}async function i(){const u=re.ScoreUI,d=new Nt;d.init(),await d.load(u,m.edit),d.uiInfo.name="ScoreEdit",d.uiInfo.left="110",d.uiInfo.top="100";const p=d.uiInfo.name,$=await d.toHTML(d.makeScoreEditItems(),d.makeScoreLog);if(e){const k=document.createElement("div");k.id=d.uiInfo.name,k.className=d.uiInfo.name,document.body.appendChild(k);const B="スコア設定",A=d.createEditorBox(B,p,$);d.addEventHandlers(A),d.addItemEventHandlers(),A.show(),d.enableResize();const J=new X;J.setAsDlg(A,B),t.add(J)}return d}(await s()).startAutoSave(),(await i()).startAutoSave();const l=document.createElement("div"),a="dockEdit";l.id=a,l.className=a,document.body.appendChild(l);let r=null;const c=await t.toHTML(a,null);if(e){const u=t.createDockBox(a,c);t.addItemClickHandlers(async d=>{t.stdApplyAction(d)}),u.show(),r=u}const h=new b;e&&(h.AddDialogs(),h.AssignIndexies(),await h.LoadAllSetting(),await h.loadSetting(r),t.InitZOrder(h),await h.ForEachAsync(u=>{const d=j.FindDialogParent(u);return d!==null&&(m.isLocal||u==="charListArea"?d.hidden=!1:d.hidden=!0),!0}),r!==null&&(r.parentElement.hidden=!m.isLocal))}const v=new $e,Ft=v.isWebRunning;Ft?(gs(),v.parseURLParams(),v.currentUserHome===""&&v.setUser(v.user1Home)):v.setUser(v.user2Home);const je=window.EVONA_CONFIG.isLocal;v.setPath();O.encodeEnable=!je;v.setImageHome(je);const fs=ys(je);v.currentUserHome;v.statusJsonPath;v.zipPrefix;v.chListFile;v.chStatusListFile;switch(v.admin){case!0:await jt(v);break;case!1:if(Ft)if(v.edit===z.none){let m=!1,e=!1;const t=new ue;t.CheckVisible=!0,t.CheckText="位置情報を初期化",t.SecretEnable=!0;const s=v.setBrowserTitle();await t.showWait(`「${s} 」モードで起動します`)===N.Secret&&(e=!0),m=t.Checked,console.log(`secretMode=[${e}]`),console.log(`cleanMode=[${m}]`);let n=W.None;if(je)n=W.Full;else if(n=W.NotRun,e){let o="";t.CheckVisible=!1,t.SecretEnable=!1,t.AuthVisible=!0,t.onAuthChecking=u=>(o=u,!0),await t.showWait("認証コードを入力してください");const l=new Date,a=("0"+(l.getMonth()+1)).slice(-2),r=("0"+l.getDate()).slice(-2),h=Math.random()>.5?`${a}${r}`:`${r}${a}`;o===h?(console.log("auth success"),n=W.Full):console.log("auth fail")}else fs&&(n=W.Limit);switch(m&&(await(await Jt()).clear(),await(await le()).clear()),v.evonaType=n,n){case W.Full:window.EVONA_CONFIG.demo=!1,await Tt(v);break;case W.Limit:window.EVONA_CONFIG.demo=!0,await Tt(v);break}}else v.setBrowserTitle(),await ps(v);break}function gs(){const m=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"||window.location.hostname.startsWith("192.168."),e=m?"./jszip_dist/jszip.min.js":"https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js",t=m?"./jsstore_dist/jsstore.min.js":"https://cdn.jsdelivr.net/npm/jsstore/dist/jsstore.min.js";window.EVONA_CONFIG={isLocal:m,workerFile:"./jsstore_dist/jsstore.worker.min.js",demo:!m};function s(i,n=!1){const o=document.createElement("script");o.src=i,n&&(o.type="module"),o.async=!1,document.head.appendChild(o)}s(e),s(t)}function ys(m){if(m)return!0;const e=document.referrer;console.log(`${e}`);const t="evona_auth",s="granted";return sessionStorage.getItem(t)===s?!0:e.includes("qiita.com")?(sessionStorage.setItem(t,s),!0):!1}
