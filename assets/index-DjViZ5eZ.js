...
`.trim()}static get spacingTable(){return"border-collapse: collapse; border-spacing: 0;"}static get spacingTr(){return`background: ${this.normBkColor};`}static sliderText(e,t,s=new Ns){return`
/* スイッチ全体のレイアウト */
.${e}-container {
  display: flex;
  align-items: center;
...
`.trim()}static get footerPadding(){return 4}}R.width="16px";R.offColor="#fff176";R.onColor="#fbc02d";R.normBkColor="#f5ecc6";class bt{constructor(){this.imgSrc="",this.imgFile="",this.alt=""}}class Ft{constructor(){this.selectedItem="",this.selectionPair=[],this.classify=""}}class j{constructor(){this.type="text",this.value="",this.placeholder="",this.disableKeyDown=!1,this.enableClicked=!1}}class Mt{constructor(){this.type="checkbox",this.value=!1,this.placeholder="",this.enableClicked=""}}class os{constructor(){this.typeInfo=new as,this.className=""}ToHTML(){let e="";switch(this.typeInfo.using.itemType){case D.Btn:e=this.typeInfo.ToButtonHTML(this.className);break;case D.Label:e=this.typeInfo.ToLableHTML(this.className);break;case D.LabelRO:e=this.typeInfo.ToLableROHTML(this.className);break;case D.Combo:e=this.typeInfo.ToComboHTML(this.className);break;case D.Input:e=this.typeInfo.ToInputHTML(this.className);break;case D.Chk:e=this.typeInfo.ToCheckHTML(this.className);break;case D.Img:const t=this.typeInfo.ToImgHTML(this.className,this.typeInfo.using.img.alt),s=`<div class="overlay" title="${this.typeInfo.toolTip}"></div>`;e=`<div class="cell-${this.className}" item-id="${this.typeInfo.using.itemId}">${t}${s}</div>`;break;case D.Plain:e=this.typeInfo.using.innerHTML;break}return e}copyTo(e){e.className=this.className,e.typeInfo.toolTip=this.typeInfo.toolTip,e.typeInfo.using.itemType=this.typeInfo.using.itemType,e.typeInfo.using.label=this.typeInfo.using.label,e.typeInfo.using.combo=this.typeInfo.using.combo,e.typeInfo.using.input=this.typeInfo.using.input,e.typeInfo.using.innerHTML=this.typeInfo.using.innerHTML,e.typeInfo.using.img=this.typeInfo.using.img}}class Me{constructor(){this.items=new Array}makeItems(e=1){this.items.length>0&&this.items.splice(0,this.items.length);for(let t=0;t<e;t++){const s=new os;this.items.push(s)}}ToHTML(){let e="";for(const t of this.items){const s=t.ToHTML();s!==""&&(this.items.length>=2?e=`${e}<div>${s}</div>`:e=s)}return e}}class Pt{constructor(){this.rowName="",this.cols=new Array}makeCols(e){this.cols.length>0&&this.cols.splice(0,this.cols.length);for(let t=0;t<e;t++){const s=new Me;s.makeItems(),this.cols.push(s)}}ToHTML(){let e="";for(const t of this.cols){let s="";s=t.ToHTML(),s!==""&&(e=`${e}<td>${s}</td>`),t.items.length===1&&t.items[0].typeInfo.using.itemType===D.LowerTable&&(e=`${e}<td></td>`)}return e}toTemplate(){const e=new Pt;e.rowName=this.rowName;for(const t of this.cols){const s=new Me;s.makeItems(t.items.length);let n=-1;for(const i of t.items){n++;const o=s.items[n];o.className=i.className,o.typeInfo.toolTip=i.typeInfo.toolTip,o.typeInfo.using.itemType=i.typeInfo.using.itemType,o.typeInfo.using.label=i.typeInfo.using.label,o.typeInfo.using.combo=i.typeInfo.using.combo,o.typeInfo.using.input=i.typeInfo.using.input,o.typeInfo.using.innerHTML=i.typeInfo.using.innerHTML,o.typeInfo.using.img=i.typeInfo.using.img}e.cols.push(s)}return e}}class z{constructor(){this.tableName="",this.rows=new Array,this.template=null,this.invalidRowIndex=-1,this.firstRowIndex=1,this.height="300",this.fontSize="0.9",this.rowIdDelimiter="_",this.clipboardText=""}makeDim(e,t){this.rows.length>0&&this.rows.splice(0,this.rows.length);for(let s=0;s<t;s++){const n=new Pt;n.makeCols(e),this.rows.push(n)}}growCell(e,t=1){const s=this.rows.length;for(let n=0;n<s;n++)this.rows[n].cols[e].makeItems(t)}getCell(e,t,s=0){return this.rows[t].cols[e].items[s]}ToHTML(e="",t){this.tableName=e;let s="",n=0;for(const a of this.rows){n++;let r="";if(r=a.ToHTML(),r!==""){const c=a.rowName!==""?` class="${a.rowName}"`:"";s=`${s}<tr ${c} item-id="${n}">${r}</tr>`}}const i=e!==""?` class="${e}"`:"",o=t!==""?` id="${t}"`:"",l=t!==""?` item-id="${t}"`:"";return s=`<table${i}${o}${l}>${s}</table>`,s}ToScrollHTML(e="",t){const s=e!==""?` class="${e}"`:"",n=t!==""?` item-id="${t}"`:"";return`<div${s}${n}>
${this.ToHTML(e,t)}
...
cursor: pointer;
        ...
        `.trim()}enableEvents(e){return this.ctlElem===null?!1:(this.htmlMaker.enableTableEvents(this.ctlElem.className),this.onSelect=e,!0)}disableEvents(){return this.ctlElem===null?!1:(this.htmlMaker.disableTableEvents(this.ctlElem.className),!0)}dispose(){super.dispose(),this.ctlElem!==null&&(this.ctlElem.parentElement?.removeChild(this.ctlElem),this.ctlElem.remove()),this.disableEvents(),this.ctlElem=null}tblCssName(){return`${this.ctlName}-tbl`}}class Es{constructor(){this.key="",this.text=""}}class $s extends Le{constructor(){super(...arguments),this.callerName="",this.htmlMaker=new H,this.table=new z,this.headerElem=null,this.footerElem=null,this.width=0,this.height=0}setGridtems(e){this.items=e}setListener(e,t,s,n="",i=""){if(this.items===void 0)return!1;this.dispose(),this.callerName=s,this.ctlName=e;const o=this.items.length,l=new z;l.makeDim(2,o);let a=0;for(const I of this.items)l.getCell(0,a).typeInfo.setLabel(`${I.text}`,!1),l.getCell(0,a).className=this.keyCssName(),l.getCell(0,a).typeInfo.using.itemId=a,l.getCell(1,a).typeInfo=I.value.items[0].typeInfo,l.getCell(1,a).className=this.valueCssName(),l.getCell(1,a).typeInfo.using.itemId=a,a++;const r=this.firstAction(e,t);if(r===null)return!1;const c=l.ToScrollHTML(e,this.tblCssName()),u=new Y;u.props.name="",u.props.id=this.tblCssName(),u.props.className=this.tblCssName(),u.props.option.setTable(c),u.props.option.onSelect=async I=>{if(this.onSelect!==void 0){const S=new oe;S.callerName=I.classify,S.result=I.selectedValue,await this.onSelect(S)}},this.htmlMaker=new H,this.htmlMaker.add(u);const p=this.htmlMaker.ToHTML();let m=document.createElement("dialog");m.className=e,m.innerHTML=p,this.ctlElem=m;let g=m.children[0];const b=this.divCssName();g.className=b,g.id=b;let d=g.children[0];return d.className=`${this.tblCssName()}`,d.id=`${this.tblCssName()}`,n!==""&&(this.headerElem=document.createElement("div"),this.headerElem.className=n,this.headerElem.id=n,m.appendChild(this.headerElem)),r.appendChild(this.ctlElem),i!==""&&(this.footerElem=document.createElement("div"),this.footerElem.className=i,this.footerElem.id=i,m.appendChild(this.footerElem)),this.table=l,!0}show(e,t){this.ctlElem!==null&&(this.ctlElem.style.left=`${e}px`,this.ctlElem.style.top=`${t}px`,this.ctlElem.show())}setFontConfig(e="0.9"){this.table.setFontConfig(e)}setSize(e="200",t="300"){let s=0;if(this.footerElem!==null){const n=this.htmlMaker.GetRect(this.footerElem);s=Number(n.height)}this.table.height=`${Number(t)-s}`,this.height=Number(t)+R.footerPadding,this.width=Number(e)}applyCss(){const e=`${this.getBaseElem().className}-flgrd-style`;if(document.getElementById(e))return;const t=this.appendStyle(e);t.textContent=`
${this.getOuterCssText()}
...
`.trim()}}class Bs{constructor(){this.itemType=D.Img,this.itemId=-1,this.label="",this.innerHTML=""}}class as{constructor(){this.toolTip="",this.using=new Bs}setButton(e){this.using.itemType=D.Btn,this.using.label=e}setLabel(e,t){this.using.itemType=t?D.Label:D.LabelRO,this.using.label=e}setCombo(e){this.using.itemType=D.Combo,this.using.combo=e}setInput(e){this.using.itemType=D.Input,this.using.input=e}setCheck(e){this.using.itemType=D.Chk,this.using.check=e}setImg(e){this.using.itemType=D.Img,this.using.img=e}setPlain(e){this.using.itemType=D.Plain,this.using.innerHTML=e}setTable(e){this.using.itemType=D.Table,this.using.innerHTML=e}setLowerTable(){this.using.itemType=D.LowerTable}ToButtonHTML(e){const t=this.using.itemId>=0?` item-id="${this.using.itemId}"`:"";return`
...
${this.htmlMaker.MakeTableScrollCss(n,s,!0)}
.${n} {
${R.spacingTable}
}
.${n} tr {
