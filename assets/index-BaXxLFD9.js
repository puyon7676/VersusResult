...
`.trim(),document.head.appendChild(a);const l=document.getElementById(this.parentName);this.htmlMaker.copyCssToInlineStyle(`.${this.parentName}`,l)}}const ne={Unknown:"Unknown",Success:"Success",Fail:"Fail",Cancel:"Cancel"};async function Lt(h){const e=h.isWebRunning,t=h.currentUserHome,s=h.chStatusListFile,n=h.chListFile,i=new gt,o=new H,a="splash-screen";o.initFullScreen(a,"エボナ データベース初期化中..."),o.applyFullScreenCss(a);const l=o.getFullScreenMsgElem(a),r=l!==null?l.innerHTML:"";if(t===h.user1Home){const f=[{ns:v.CnsRed,nsName:"赤属性"},{ns:v.CnsBlue,nsName:"青属性"},{ns:v.CnsGreen,nsName:"緑属性"},{ns:v.CnsYellow,nsName:"黄属性"},{ns:v.CnsViolet,nsName:"紫属性"}],T=await new oe().LoadList(n);for(const C of f)l!==null&&(l.innerHTML=`${r} ${C.nsName}`),await i.setupNs(C.ns,h,T)}if(t===h.user2Home){const f=[{ns:v.CnsBlue,nsName:"藍属性"},{ns:v.CnsRed,nsName:"紅属性"},{ns:v.CnsGreen,nsName:"翠属性"},{ns:v.CnsYellow,nsName:"黄属性"},{ns:v.CnsWhite,nsName:"天属性"},{ns:v.CnsBlack,nsName:"冥属性"}],T=await new oe().LoadList(n);for(const C of f)l!==null&&(l.innerHTML=`${r} ${C.nsName}`),await i.setupNs(C.ns,h,T)}l!==null&&(l.innerHTML="UI 初期化中 ...");const c=new Be,m=new ke;await m.loadDB(s);async function d(){async function f(){return await new oe().LoadList(n)}const u=await f();u.uiInfo.name="charListArea",u.uiInfo.left="300",u.uiInfo.top="100";const T=u.uiInfo.name,C=await u.toHTML(i);if(e){const $="キャラ選択",p=u.createSelectorBox($,T,C);u.addEventHandlers(p),u.addItemEventHandlers(),u.enableLazyImages(i),p.show();const k=new Q;k.setAsDlg(p,$),c.add(k)}return u}const w=await d();async function S(){const f=new kt;await f.load(),f.uiInfo.name="CharSummary",f.uiInfo.left="400",f.uiInfo.top="100";const u=f.uiInfo.name,T=await f.toHTML();if(e){const C="キャラ一覧",$=f.createSummaryBox(C,u,T);await f.updateCharInfos(i),f.addEventHandlers($,async k=>{k.selectCh=w.selectedCh;const R=await i.getImageUrlBy(w.selectedCh.iconFileName,w.selectedCh.ns);if(R===null)k.selectedImg="",k.cancel=!0;else{k.selectedImg=R,k.cancel=!1;const x=await m.getStatus(w.selectedCh.idAsText);console.log(x),k.scoreSet=x}console.log(`selected ch = ${k.selectCh.name}`)}),f.addItemEventHandlers(),f.enableLazyImages(i),$.show();const p=new Q;p.setAsDlg($,C),c.add(p)}return f}const D=await S();async function z(f,u,T,C,$){const p=new Tt;p.Init(m),p.formation.uiInfo.name=f,p.formation.uiInfo.left=`${u}`,p.formation.uiInfo.top=`${T}`;const k=p.formation.uiInfo.name,R=await p.toHTML(k,i),x=p.toGridHTML();if(e){const _=p.createFormationBox(C,k,R,x,$);p.addEventHandlers(_),p.addItemEventkHandlers(async N=>{N.selectCh=w.selectedCh;const A=await i.getImageUrlBy(w.selectedCh.iconFileName,w.selectedCh.ns);if(A===null)return;N.selectedImg=A,N.item.isEmpty=!1,console.log(`selected ch = ${N.selectCh.name}`);const W=Le.HiLv;E.combatPairs.get(W),N.uiName===V&&await E.replaceChar(W,O.Player,N.item,N.selectCh,i),N.uiName===re&&await E.replaceChar(W,O.Enemy,N.item,N.selectCh,i),await E.replaceJudge(i)},async N=>{const A=await i.getImageUrlBy(N.selectedImg,w.selectedCh.ns);if(A===null)return;N.selectedImg=A,N.item.isEmpty=!0,console.log(`empty ch = ${N.selectedImg}`);const W=Le.HiLv;E.combatPairs.get(W),N.uiName===V&&await E.replaceChar(W,O.Player,N.item,N.selectCh,i),N.uiName===re&&await E.replaceChar(W,O.Enemy,N.item,N.selectCh,i),await E.replaceJudge(i)},async N=>{console.log(`selected ch = ${N.item.ch.name}`);const A=new st;A.selectCh=N.item.ch,A.selectedImg=N.selectedImg,A.scoreSet=N.scoreSet,D.charStock(A)},async N=>{const A=Ke();console.log(A),await A.replaceJudge(i)}),p.enableLazyImages(i),_.show();const he=new Q;he.setAsDlg(_,C),c.add(he)}return p}const V="playerForm",re="enemyForm",Se=await z(V,100,100,"自編成",!0),Te=await z(re,100,200,"敵編成",!1);async function lt(f,u,T,C){E.uiInfo.name=f,E.uiInfo.left=`${u}`,E.uiInfo.top=`${T}`;const $=await E.toHTML("combatTable",i),p=E.createCombatBox(C,f,$);E.enableLazyImages(i),await E.replaceJudge(i),p.show();const k=new Q;k.setAsDlg(p,C),c.add(k)}const E=Ke();await lt("combatForm",120,300,"対戦予想");const Oe="保存";{const f=new Q;f.setAsMenu(Oe),c.add(f)}const Je="復帰";{const f=new Q;f.setAsMenu(Je),c.add(f)}let ze=null;const rt=await c.toHTML("dockForm",i);if(e){const f=c.createDockBox("dockForm",rt,!h.isLocal);c.addItemClickHandlers(async u=>{if(u.item.dlgParent===null){u.cancel=!0;return}console.log(`selected item = [${u.item.toolTip}::${u.item.dockType}]`),u.item.isUIType&&(new y().MoveHiestLayer(u.item.dlgParent),u.item.dlgParent.hidden&&(u.item.dlgParent.hidden=!1)),u.item.isMenuType&&(u.item.toolTip===Oe&&await ct(),u.item.toolTip===Je&&await mt(async T=>{if(console.log(`[loadedResult] ${T}`),T!==ne.Success)return;const C=Y.fromJsonInst(ce),$=Y.fromJsonInst(me);ce=null,me=null,await Se.Setup(C,i),await Te.Setup($,i)}))}),c.enableLazyImages(i),f.show(),ze=f}const je="playerForm.json",Ve="enemyForm.json",Ge="dockForm.json";async function ct(){c.InitZOrder(G);const f=Y.toJsonText(Se.formation),u=Y.toJsonText(Te.formation),T=we.toJsonText(c),C=new window.JSZip;C.file(je,f),C.file(Ve,u),C.file(Ge,T);const $=await C.generateAsync({type:"blob"}),p="gameConfig.zip",k=URL.createObjectURL($),R=document.createElement("a");R.href=k,R.download=p,R.click(),URL.revokeObjectURL(k),console.log("saved!")}let We=null,ce=null,me=null;async function mt(f){const u=document.createElement("input");return u.type="file",u.accept=".zip",u.addEventListener("cancel",()=>(console.log("Cancelled."),ne.Cancel)),u.addEventListener("change",async()=>{if(u.files.length==1){console.log("File selected: ",u.files[0].name);const C=await u.files[0].arrayBuffer(),p=await new window.JSZip().loadAsync(C);async function k(x){const _=p.file(x);if(_){const he=await _.async("string"),N=JSON.parse(he);return console.log(N),N}}{const x=await k(Ge);x&&(We=x)}{const x=await k(je);x&&(ce=x)}{const x=await k(Ve);x&&(me=x)}const R=We!==null&&ce!==null&&me!==null?ne.Success:ne.Fail;f(R)}}),u.click(),ne.Unknown}const G=new y;e&&(G.AddDialogs(),G.AssignIndexies(),await G.LoadAllSetting(),await G.loadSetting(ze),c.InitZOrder(G),await G.ForEachAsync(f=>{const u=J.FindDialogParent(f);return u!==null&&(h.isLocal||f==="charListArea"?u.hidden=!1:u.hidden=!0),!0})),o.hideFullScreenCss(a);function qe(f){const u=new Ue,T=new Pe,C=f.formation;C.uiInfo.name,C.uiInfo.name;let $=0;for(const p of C.items){const k=f.scsList[$];$++;const R=Math.ceil(k.stdScore),x=new ot(p.ch.id,p.ch.name,p.isEmpty,R);x.allAvailable=k.allAvailable,T.Add(x)}return u.Add(T),u.debug(),u}function Ke(){const f=new xt,u=qe(Se),T=qe(Te),C=new De;C.setScoreItems(u.groupRows[0].columns),C.boost=100;const $=new De;$.setScoreItems(T.groupRows[0].columns),$.boost=100;const p=new Mt;p.setPlayer(C),p.setEnemy($),f.setPair(Le.HiLv,p),f.calcCombatScore();for(const[k,R]of f.combatPairs){const x=R.judge(U.None);console.log(`judge=[${x}]`)}return f}}class vt{constructor(){this.intervalMs=1e3,this.isOneShot=!0,this.intervalTimer=null}start(e,t){this.intervalTimer!==null&&this.clear(),this.intervalTimer=setTimeout(async()=>{t!==void 0&&(await t(e)||this.clear()),this.isOneShot===!0&&this.clear()},this.intervalMs)}clear(){clearTimeout(this.intervalTimer),this.intervalTimer=null}}class _e{constructor(){this.key="",this.text="",this.comment=""}}class Et{constructor(){this.itemList=new Array,this.uiInfo=new ee,this.parentName="",this.saveTimer=null,this.onSave=async e=>(console.log(`${e.parentName}`),!1)}init(){for(let e=0;e<10;e++)this.itemList.push(new _e)}async startAutoSave(){this.saveTimer=new vt,this.saveTimer.intervalMs=1e3,this.saveTimer.isOneShot=!1,this.saveTimer.start(this,this.onSave)}setRow(e,t,s){const n=new $e;n.placeholder=e.key===""?"キー":e.key,s.getCell(0,t).typeInfo.setInput(n),s.getCell(0,t).className=this.itemKeyCssClassName(),s.getCell(0,t).typeInfo.using.itemId=t;const i=new $e;i.placeholder=e.text===""?"文字列":e.text,s.getCell(1,t).typeInfo.setInput(i),s.getCell(1,t).className=this.itemTextCssClassName(),s.getCell(1,t).typeInfo.using.itemId=t;const o=new $e;o.placeholder=e.comment===""?"コメント":e.comment,s.getCell(2,t).typeInfo.setInput(o),s.getCell(2,t).className=this.itemCommentCssClassName(),s.getCell(2,t).typeInfo.using.itemId=t,s.getCell(3,t).typeInfo.setButton("削除"),s.getCell(3,t).className=this.itemDeleteCssClassName(),s.getCell(3,t).typeInfo.using.itemId=t}toHTML(){if(!this.itemList)return"";const e=new ae;if(this.itemList.length>=1)e.makeDim(4,this.itemList.length),this.itemList.forEach((i,o)=>{this.setRow(i,o,e)}),e.makeRowTemplate(this.tableRowCssClassName()),this.table=e;else{e.makeDim(4,1);const i=new _e;this.setRow(i,0,e),e.makeRowTemplate(this.tableRowCssClassName()),e.clearRows(),this.table=e}const t=this.tableCssClassName(),s=this.table.ToScrollHTML(t,"");this.htmlMaker=new H;const n=new F;return n.props.name="",n.props.id="0",n.props.className=t,n.props.option.setTable(s),n.props.option.onSelect=async i=>{switch(console.log(`classify = ${i.classify} targetId = ${i.targetId}`),i.classify){case this.itemKeyCssClassName():break;case this.itemTextCssClassName():break;case this.itemCommentCssClassName():break;case this.itemDeleteCssClassName():const o=new Z;o.setParent(this.dlgCssClassName());let a=b.None;switch(o.setYesNo(),a=await o.showWait(`${i.targetId} を削除しますか？`),a){case b.Yes:break;case b.No:return;case b.Cancel:return}const l=parseInt(i.targetId);l>=1&&(e.deleteRow(l),e.redimAllRows(),this.itemList.splice(l-this.table.firstRowIndex,1));break}},this.htmlMaker.add(n),this.htmlMaker.ToHTML()}createSummaryBox(e,t,s){this.parentName=t;const n=this.htmlMaker.MakeDefaultButtonsHTML(`
...
${this.htmlMaker.MakeDefaultButtonsCss()}
`.trim(),document.head.appendChild(s);const n=document.getElementById(this.parentName);this.htmlMaker.copyCssToInlineStyle(`.${this.parentName}`,n)}}async function Rt(h){if(!h.isWebRunning)return;const e=h.isWebRunning;async function t(){const n=new Et;n.init(),n.uiInfo.name="ResourceEdit",n.uiInfo.left="110",n.uiInfo.top="10";const i=n.uiInfo.name,o=await n.toHTML();if(e){const a=document.createElement("div");a.id=n.uiInfo.name,a.className=n.uiInfo.name,document.body.appendChild(a);const r=n.createSummaryBox("文字列リソース",i,o);n.addEventHandlers(r),n.addItemEventHandlers(),r.show()}return n}(await t()).startAutoSave()}const L=new le,at=L.isWebRunning;at?(Ht(),L.parseURLParams(),L.currentUserHome===""&&L.setUser(L.user1Home)):L.setUser(L.user2Home);L.setPath();L.setImageHome(window.EVONA_CONFIG.isLocal);L.currentUserHome;L.statusJsonPath;L.zipPrefix;L.chListFile;L.chStatusListFile;switch(L.admin){case!0:await pt(L);break;case!1:if(at)if(L.resMode===""){let h=!1,e=!1;const t=new Z;t.CheckVisible=!0,t.CheckText="位置情報を初期化",t.SecretEnable=!0;const s=L.setBrowserTitle();await t.showWait(`「${s} 」モードで起動します`)===b.Secret&&(e=!0),h=t.Checked,console.log(`secretMode=[${e}]`),console.log(`cleanMode=[${h}]`),h&&(await(await ut()).clear(),await(await Ee()).clear()),e&&(window.EVONA_CONFIG.demo=!1),await Lt(L)}else await Rt(L);break}function Ht(){const h=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"||window.location.hostname.startsWith("192.168."),e=h?"./jszip_dist/jszip.min.js":"https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js",t=h?"./jsstore_dist/jsstore.min.js":"https://cdn.jsdelivr.net/npm/jsstore/dist/jsstore.min.js";window.EVONA_CONFIG={isLocal:h,workerFile:"./jsstore_dist/jsstore.worker.min.js",demo:!h};function s(n,i=!1){const o=document.createElement("script");o.src=n,i&&(o.type="module"),o.async=!1,document.head.appendChild(o)}s(e),s(t)}
