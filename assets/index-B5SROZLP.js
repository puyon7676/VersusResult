...
${this.htmlMaker.MakeDefaultDialogParentCss(this.parentName,60,10,I.ignoreIndex)}
...
`.trim(),document.head.appendChild(a);const l=document.getElementById(this.parentName);this.htmlMaker.copyCssToInlineStyle(`.${this.parentName}`,l)}}const K={Unknown:"Unknown",Success:"Success",Fail:"Fail",Cancel:"Cancel"};async function ee(u){const t=u.isWebRunning,e=u.currentUserHome,s=u.chStatusListFile,n=u.chListFile,i=new Jt,o=new D,a="splash-screen";o.initFullScreen(a,"エボナ データベース初期化中..."),o.applyFullScreenCss(a);const l=window.EVONA_CONFIG.workerFile,r=new Ft;r.init(l),await r.initDb();const c=o.getFullScreenMsgElem(a),d=c!==null?c.innerHTML:"";if(e===u.user1Home){const g=[{ns:H.CnsRed,nsName:"赤属性",zipName:"puyoQueRed.zip"},{ns:H.CnsBlue,nsName:"青属性",zipName:"puyoQueBlue.zip"},{ns:H.CnsGreen,nsName:"緑属性",zipName:"puyoQueGreen.zip"},{ns:H.CnsYellow,nsName:"黄属性",zipName:"puyoQueYellow.zip"},{ns:H.CnsViolet,nsName:"紫属性",zipName:"puyoQueViolet.zip"}];for(const h of g)c!==null&&(c.innerHTML=`${d} ${h.nsName}`),await i.setupNs(h.nsName,`${e}${h.zipName}`,r)}e===u.user2Home&&await i.setupNs(H.None,`${e}Image_meme.zip`,r),c!==null&&(c.innerHTML="UI 初期化中 ...");const m=new yt,b=new ct;await b.loadDB(s);async function S(){async function g(){return await new gt().LoadList(n)}const h=await g();h.uiInfo.name="charListArea",h.uiInfo.left="300",h.uiInfo.top="100";const w=await h.toHTML(i);if(t){const y="キャラ選択",f=h.createSelectorBox(y,"charListArea",w);h.addEventHandlers(f),h.addItemEventHandlers(),h.enableLazyImages(i),f.show();const N=new W;N.setAsDlg(f,y),m.add(N)}return h}const R=await S();async function A(g,h,w,y){const f=new Xt;f.Init(b),f.formation.uiInfo.name=g,f.formation.uiInfo.left=`${h}`,f.formation.uiInfo.top=`${w}`;const N=f.formation.uiInfo.name,$=await f.toHTML(N,i),v=f.toGridHTML();if(t){const M=f.createFormationBox(y,N,$,v);f.addEventHandlers(M),f.addItemEventkHandlers(C=>{console.log(`selected ch = ${C.item.ch.name}`)},async C=>{C.selectCh=R.selectedCh;const F=await i.getImageUrlBy(R.selectedCh.iconFileName,R.selectedCh.ns);if(F===null)return;C.selectedImg=F,C.item.isEmpty=!1,console.log(`selected ch = ${C.selectCh.name}`);const z=dt.HiLv;T.combatPairs.get(z),C.uiName===U&&await T.replaceChar(z,B.Player,C.item,C.selectCh,i),C.uiName===X&&await T.replaceChar(z,B.Enemy,C.item,C.selectCh,i),await T.replaceJudge(i)},async C=>{const F=await i.getImageUrlBy(C.selectedImg,R.selectedCh.ns);if(F===null)return;C.selectedImg=F,C.item.isEmpty=!0,console.log(`empty ch = ${C.selectedImg}`);const z=dt.HiLv;T.combatPairs.get(z),C.uiName===U&&await T.replaceChar(z,B.Player,C.item,C.selectCh,i),C.uiName===X&&await T.replaceChar(z,B.Enemy,C.item,C.selectCh,i),await T.replaceJudge(i)},async C=>{const F=$t();console.log(F),await F.replaceJudge(i)}),f.enableLazyImages(i),M.show();const G=new W;G.setAsDlg(M,y),m.add(G)}return f}const U="playerForm",X="enemyForm",mt=await A(U,100,100,"自編成"),ht=await A(X,100,200,"敵編成");async function Rt(g,h,w,y){T.uiInfo.name=g,T.uiInfo.left=`${h}`,T.uiInfo.top=`${w}`;const f=await T.toHTML("combatTable",i),N=T.createCombatBox(y,g,f);T.enableLazyImages(i),await T.replaceJudge(i),N.show();const $=new W;$.setAsDlg(N,y),m.add($)}const T=$t();await Rt("combatForm",120,300,"対戦予想");const It="保存";{const g=new W;g.setAsMenu(It),m.add(g)}const Nt="復帰";{const g=new W;g.setAsMenu(Nt),m.add(g)}const At=await m.toHTML("dockForm",i);if(t){const g=m.createDockBox("dockForm",At);m.addItemClickHandlers(async h=>{if(h.item.dlgParent===null){h.cancel=!0;return}console.log(`selected item = [${h.item.toolTip}::${h.item.dockType}]`),h.item.isUIType&&(new I().MoveHiestLayer(h.item.dlgParent),h.item.dlgParent.hidden&&(h.item.dlgParent.hidden=!1)),h.item.isMenuType&&(h.item.toolTip===It&&await Ut(),h.item.toolTip===Nt&&await zt(async w=>{if(console.log(`[loadedResult] ${w}`),w!==K.Success)return;const y=j.fromJsonInst(Y),f=j.fromJsonInst(Z);Y=null,Z=null,await mt.Setup(y,i),await ht.Setup(f,i)}))}),m.enableLazyImages(i),g.show()}const Mt="playerForm.json",wt="enemyForm.json",kt="dockForm.json";async function Ut(){m.InitZOrder(tt);const g=j.toJsonText(mt.formation),h=j.toJsonText(ht.formation),w=it.toJsonText(m),y=new window.JSZip;y.file(Mt,g),y.file(wt,h),y.file(kt,w);const f=await y.generateAsync({type:"blob"}),N="gameConfig.zip",$=URL.createObjectURL(f),v=document.createElement("a");v.href=$,v.download=N,v.click(),URL.revokeObjectURL($),console.log("saved!")}let St=null,Y=null,Z=null;async function zt(g){const h=document.createElement("input");return h.type="file",h.accept=".zip",h.addEventListener("cancel",()=>(console.log("Cancelled."),K.Cancel)),h.addEventListener("change",async()=>{if(h.files.length==1){console.log("File selected: ",h.files[0].name);const y=await h.files[0].arrayBuffer(),N=await new window.JSZip().loadAsync(y);async function $(M){const G=N.file(M);if(G){const C=await G.async("string"),F=JSON.parse(C);return console.log(F),F}}{const M=await $(kt);M&&(St=M)}{const M=await $(Mt);M&&(Y=M)}{const M=await $(wt);M&&(Z=M)}const v=St!==null&&Y!==null&&Z!==null?K.Success:K.Fail;g(v)}}),h.click(),K.Unknown}const tt=new I;t&&(tt.AddDialogs(),tt.AssignIndexies(),m.InitZOrder(tt)),o.hideFullScreenCss(a);function Tt(g){const h=new bt,w=new Ct,y=g.formation;y.uiInfo.name,y.uiInfo.name;let f=0;for(const N of y.items){const $=g.scsList[f];f++;const v=Math.ceil($.stdScore),M=new Bt(N.ch.id,N.ch.name,N.isEmpty,v);M.allAvailable=$.allAvailable,w.Add(M)}return h.Add(w),h.debug(),h}function $t(){const g=new te,h=Tt(mt),w=Tt(ht),y=new ft;y.setScoreItems(h.groupRows[0].columns),y.boost=100;const f=new ft;f.setScoreItems(w.groupRows[0].columns),f.boost=100;const N=new Zt;N.setPlayer(y),N.setEnemy(f),g.setPair(dt.HiLv,N),g.calcCombatScore();for(const[$,v]of g.combatPairs){const M=v.judge(E.None);console.log(`judge=[${M}]`)}return g}}const k=new q,se=k.isWebRunning;se?(k.parseURLParams(),k.currentUserHome===""&&k.setUser(k.user1Home)):k.setUser(k.user2Home);k.currentUserHome;k.setPath();k.statusJsonPath;k.zipPrefix;k.chListFile;k.chStatusListFile;switch(k.admin){case!0:await Ot(k);break;case!1:await ee(k);break}
