...
}`}}class st{constructor(){this.itemType=b.Img,this.itemId=-1,this.label="",this.innerHTML=""}}class ze{constructor(){this.toolTip="",this.using=new st}setButton(e){this.using.itemType=b.Btn,this.using.label=e}setLabel(e,t){this.using.itemType=t?b.Label:b.LabelRO,this.using.label=e}setCombo(e){this.using.itemType=b.Combo,this.using.combo=e}setInput(e){this.using.itemType=b.Input,this.using.input=e}setImg(e){this.using.itemType=b.Img,this.using.img=e}setPlain(e){this.using.itemType=b.Plain,this.using.innerHTML=e}setTable(e){this.using.itemType=b.Table,this.using.innerHTML=e}ToButtonHTML(e){const t=this.using.itemId>=0?` item-id="${this.using.itemId}"`:"";return`
...
<img class="${e}" ${s} alt="${t}">
`.trim()}ToOverlayHTML(){return`<div class="overlay" title="${this.toolTip}"></div>`}}class nt{constructor(){this.name="",this.id="",this.className="",this.option=new ze}}class F{constructor(){this.props=new nt}ToHTML(e){let t="",s="";switch(e.option.using.itemType){case b.Label:t=e.option.ToLableHTML(e.className),s=e.option.ToOverlayHTML();break;case b.LabelRO:t=e.option.ToLableROHTML(e.className);break;case b.Plain:t=`
...
`.trim(),document.head.appendChild(r);const c=document.getElementById(this.parentName);this.htmlMakerChSel.copyCssToInlineStyle(`.${this.parentName}`,c)}}class we{constructor(){this.cancel=!1}}class ut{constructor(){this.uiName="",this.cancel=!1}}class fe{constructor(){this.ch=new q,this.isEmpty=!0,this.TextMap=""}static toJsonText(e){const t=q.toJsonInst(e.ch);return JSON.stringify(t,null,2)}static toJsonInst(e){const t=new fe;t.ch=q.toJsonInst(e.ch),t.isEmpty=e.isEmpty;const s=Object.fromEntries(e.details.TextMap);return t.TextMap=JSON.stringify(s,null,2),t}static fromJsonInst(e){const t=new je;t.ch=q.fromJsonInst(e.ch),t.isEmpty=e.isEmpty;const s=new Map(Object.entries(JSON.parse(e.TextMap)));return t.details.TextMap=s,t}}class V{static toJsonText(e){const t=V.toJsonInst(e);return JSON.stringify(t,null,2)}static toJsonInst(e){const t=new V;t.items=new Array;for(const s of e.items)t.items.push(fe.toJsonInst(s));return t}static fromJsonInst(e){const t=new qe;t.items=new Array;for(const s of e.items)t.items.push(fe.fromJsonInst(s));return t}}class Ve{constructor(e=0,t="",s=!0,n=0){this.ch=new X,this.isEmpty=!0,this.score=0,this.allAvailable=!1,this.ch.id=e,this.ch.name=t,this.isEmpty=s,this.score=n}}class Le{Add(e){if(e===void 0){console.log("[VersusGroupRow::Add] scItem === undefined");return}this.columns===void 0&&(this.columns=[]),this.columns.push(e)}Count(){return this.columns.length}debug(){if(this.columns===void 0){console.log("[VersusGroupRow::debug] this.columns === undefined");return}for(const e of this.columns)console.log(`[${e.ch.name}]	score=[${e.score}]`)}}Le.defNumColumn=5;class ve{Add(e){this.groupRows===void 0&&(this.groupRows=[]),this.groupRows.push(e)}Count(){return this.groupRows.length}debug(){if(this.groupRows===void 0)console.log("this.groupRows === undefined");else for(const e of this.groupRows)e.debug()}async loadJson(e){const n=(await new Y().loadJson(e)).groupRows.map(o=>Object.assign(new Le,o)),i=new ve;return i.groupRows=n,i}}const A={None:"None",Player:"Player",Enemy:"Enemy"},P={None:"None",Attr:"Attr",Role:"Role"},Ie={HiLv:"HiLv"},R={Likely:"Likely",Uncertain:"Uncertain",Wishful:"Wishful"};class ge{}ge.Likely=.9;ge.Uncertain=.64;class Te{constructor(){this.scoreItems=[],this.formationType=A.None,this.boost=0}get imgPrefix(){return this.formationType===A.Player?"ftPlayer":"ftEnemy"}get judgeWidth(){return 48}setScoreItems(e){if(e!==void 0){this.scoreItems=new Array;for(const t of e){const s=new Ve(t.ch.id,t.ch.name,t.isEmpty,t.score);this.scoreItems.push(s)}}}async toCharHTML(e,t){const s=t.ch.ns===v.None?"":t.ch.ns,n=await e.getImageUrlBy(t.ch.iconFileName,s);let i=this.scoreToolTip(t);return i!==""&&(i=`title="${i}"`),`
<img class=${this.charCssClassName()}
  src="${n}"
  ${i}>
`.trim()}scoreToolTip(e){return e.isEmpty?"":`${e.ch.name}
score=${e.score}`}async toJudgeHTML(e,t){const s=this.toJudgeFileURL(e,t);return`<img class=${this.judgeCssClassName()} src="${s}" width="48">`}async toJudgeFileURL(e,t){let s=null;switch(t){case R.Likely:s=await e.getImageUrlBy("win.png",e.AnyNs);break;case R.Uncertain:s=await e.getImageUrlBy("even.png",e.AnyNs);break;case R.Wishful:s=await e.getImageUrlBy("lost.png",e.AnyNs);break;default:return""}return s}charCssClassName(){return"combat-char"}judgeCssClassName(){return"combat-judge"}get combatScore(){if(this.scoreItems===void 0)return 0;const e=this.scoreItems.reduce((s,n)=>n.isEmpty?s:s+n.score,0),t=this.boost===0?1:this.boost;return Math.ceil(e*t/100)}}class dt{constructor(){this.hasCombo=!1,this.winRate=new Map}setPlayer(e){e.formationType=A.Player,this.player=e}setEnemy(e){e.formationType=A.Enemy,this.enemy=e}judge(e){const s=this.winRate.get(e);return s>=ge.Likely?R.Likely:s>=ge.Uncertain?R.Uncertain:R.Wishful}judgeForEnemy(e){switch(e){case R.Likely:return R.Wishful;case R.Uncertain:return R.Uncertain;case R.Wishful:return R.Likely}}}class pt{constructor(){this.combatPairs=new Map,this.emptyFile="plus.png",this.parentName="",this.uiInfo=new K}setPair(e,t){this.combatPairs.set(e,t)}calcCombatScore(){for(const[e,t]of this.combatPairs){if(t.player===void 0||t.enemy===void 0)continue;const s=[P.None,P.Attr,P.Role];for(const n of s){let i=t.player.combatScore,o=t.enemy.combatScore;t.winRate.set(n,o!==0?i/o:1)}}}async replaceChar(e,t,s,n,i){const o=this.combatPairs.get(e);let a;if(t===A.Player?a=o?.player:t===A.Enemy&&(a=o?.enemy),a===void 0)return!1;const l=s.itemID;if(l<0)return!1;const r=l-1;a=a;const h=`${a.imgPrefix}${l}`,d=a.scoreItems[r],I=d.ch;s.isEmpty?(I.id=0,I.name="",console.log("set empty")):(I.id=n.id,I.name=n.name,console.log(`set char ${n.id}:${n.name}`)),d.isEmpty=s.isEmpty;const L=I.ns===v.None?"":I.ns,G=await i.getImageUrlBy(I.iconFileName,L);if(G===null)return!1;const O=new D,J=this.outerCssClassName();return O.ReplaceImg(J,h,G),O.ReplaceImgToolTip(J,h,a.scoreToolTip(d)),!0}async replaceJudge(e){async function t(i,o){const a=await i.toJudgeFileURL(e,o);if(a===null)return;const l=i.imgPrefix;for(let r=0;r<i.scoreItems.length;r++){const c=`${l}${r+1}`,h=s.FindImgsByID(n,c);if(h===null||h.length<=1){console.error("fail on judge marker");continue}const d=h[1];s.SetImgSrc(d,a),i.scoreItems[r].isEmpty?s.SetImgSize(d,0,0):s.SetImgSize(d,i.judgeWidth,i.judgeWidth);const I=i.scoreItems[r];s.ReplaceImgToolTip(n,c,i.scoreToolTip(I))}}const s=new D,n=this.outerCssClassName();for(const[i,o]of this.combatPairs){if(o.player===void 0||o.enemy===void 0)continue;const a=[P.None,P.Attr,P.Role];for(const l of a){if(l!==P.None)continue;const r=o.judge(l),c=o.judgeForEnemy(r);await t(o.player,r),await t(o.enemy,c)}}}async toHTML(e,t){const s=document.createElement("table");s.id=e;const n=document.createElement("tbody");s?.appendChild(n);async function i(a,l){const r=document.createElement("tr");n?.appendChild(r);const c=await l.toJudgeHTML(t,a),h=l.imgPrefix;let d=0;for(const I of l.scoreItems){d++;const L=await l.toCharHTML(t,I),G=`
...
`.trim(),document.head.appendChild(a);const l=document.getElementById(this.parentName);this.htmlMaker.copyCssToInlineStyle(`.${this.parentName}`,l)}}const Z={Unknown:"Unknown",Success:"Success",Fail:"Fail",Cancel:"Cancel"};async function ft(m){const e=m.isWebRunning,t=m.currentUserHome,s=m.chStatusListFile,n=m.chListFile,i=new et,o=new D,a="splash-screen";o.initFullScreen(a,"エボナ データベース初期化中..."),o.applyFullScreenCss(a);const l=o.getFullScreenMsgElem(a),r=l!==null?l.innerHTML:"";if(t===m.user1Home){const p=[{ns:v.CnsRed,nsName:"赤属性"},{ns:v.CnsBlue,nsName:"青属性"},{ns:v.CnsGreen,nsName:"緑属性"},{ns:v.CnsYellow,nsName:"黄属性"},{ns:v.CnsViolet,nsName:"紫属性"}],k=await new te().LoadList(n);for(const f of p)l!==null&&(l.innerHTML=`${r} ${f.nsName}`),await i.setupNs(f.ns,t,k)}if(t===m.user2Home){const p=[{ns:v.CnsBlue,nsName:"藍属性"},{ns:v.CnsRed,nsName:"紅属性"},{ns:v.CnsGreen,nsName:"翠属性"},{ns:v.CnsYellow,nsName:"黄属性"},{ns:v.CnsWhite,nsName:"天属性"},{ns:v.CnsBlack,nsName:"冥属性"}],k=await new te().LoadList(n);for(const f of p)l!==null&&(l.innerHTML=`${r} ${f.nsName}`),await i.setupNs(f.ns,t,k)}l!==null&&(l.innerHTML="UI 初期化中 ...");const c=new Se,h=new ye;await h.loadDB(s);async function d(){async function p(){return await new te().LoadList(n)}const u=await p();u.uiInfo.name="charListArea",u.uiInfo.left="300",u.uiInfo.top="100";const k=await u.toHTML(i);if(e){const f="キャラ選択",y=u.createSelectorBox(f,"charListArea",k);u.addEventHandlers(y),u.addItemEventHandlers(),u.enableLazyImages(i),y.show();const T=new W;T.setAsDlg(y,f),c.add(T)}return u}const I=await d();async function L(){const p=new rt;p.uiInfo.name="CharSummary",p.uiInfo.left="400",p.uiInfo.top="100";const u=p.uiInfo.name,k=await p.toHTML();if(e){const f="キャラ一覧",y=p.createSummaryBox(f,u,k);p.addItemEventHandlers(),p.enableLazyImages(i),y.show();const T=new W;T.setAsDlg(y,f),c.add(T)}return p}const G=await L();async function O(p,u,k,f){const y=new ht;y.Init(h),y.formation.uiInfo.name=p,y.formation.uiInfo.left=`${u}`,y.formation.uiInfo.top=`${k}`;const T=y.formation.uiInfo.name,H=await y.toHTML(T,i),B=y.toGridHTML();if(e){const S=y.createFormationBox(f,T,H,B);y.addEventHandlers(S),y.addItemEventkHandlers(async N=>{N.selectCh=I.selectedCh;const x=await i.getImageUrlBy(I.selectedCh.iconFileName,I.selectedCh.ns);if(x===null)return;N.selectedImg=x,N.item.isEmpty=!1,console.log(`selected ch = ${N.selectCh.name}`);const z=Ie.HiLv;E.combatPairs.get(z),N.uiName===J&&await E.replaceChar(z,A.Player,N.item,N.selectCh,i),N.uiName===ie&&await E.replaceChar(z,A.Enemy,N.item,N.selectCh,i),await E.replaceJudge(i)},async N=>{const x=await i.getImageUrlBy(N.selectedImg,I.selectedCh.ns);if(x===null)return;N.selectedImg=x,N.item.isEmpty=!0,console.log(`empty ch = ${N.selectedImg}`);const z=Ie.HiLv;E.combatPairs.get(z),N.uiName===J&&await E.replaceChar(z,A.Player,N.item,N.selectCh,i),N.uiName===ie&&await E.replaceChar(z,A.Enemy,N.item,N.selectCh,i),await E.replaceJudge(i)},async N=>{console.log(`selected ch = ${N.item.ch.name}`);const x=new ct;x.selectCh=N.item.ch,x.selectedImg=N.selectedImg,x.scoreSet=N.scoreSet,G.charStock(x)},async N=>{const x=Pe();console.log(x),await x.replaceJudge(i)}),y.enableLazyImages(i),S.show();const _=new W;_.setAsDlg(S,f),c.add(_)}return y}const J="playerForm",ie="enemyForm",Ce=await O(J,100,100,"自編成"),be=await O(ie,100,200,"敵編成");async function We(p,u,k,f){E.uiInfo.name=p,E.uiInfo.left=`${u}`,E.uiInfo.top=`${k}`;const y=await E.toHTML("combatTable",i),T=E.createCombatBox(f,p,y);E.enableLazyImages(i),await E.replaceJudge(i),T.show();const H=new W;H.setAsDlg(T,f),c.add(H)}const E=Pe();await We("combatForm",120,300,"対戦予想");const xe="保存";{const p=new W;p.setAsMenu(xe),c.add(p)}const Ee="復帰";{const p=new W;p.setAsMenu(Ee),c.add(p)}const Ye=await c.toHTML("dockForm",i);if(e){const p=c.createDockBox("dockForm",Ye);c.addItemClickHandlers(async u=>{if(u.item.dlgParent===null){u.cancel=!0;return}console.log(`selected item = [${u.item.toolTip}::${u.item.dockType}]`),u.item.isUIType&&(new M().MoveHiestLayer(u.item.dlgParent),u.item.dlgParent.hidden&&(u.item.dlgParent.hidden=!1)),u.item.isMenuType&&(u.item.toolTip===xe&&await Ke(),u.item.toolTip===Ee&&await Xe(async k=>{if(console.log(`[loadedResult] ${k}`),k!==Z.Success)return;const f=V.fromJsonInst(oe),y=V.fromJsonInst(ae);oe=null,ae=null,await Ce.Setup(f,i),await be.Setup(y,i)}))}),c.enableLazyImages(i),p.show()}const He="playerForm.json",Re="enemyForm.json",Fe="dockForm.json";async function Ke(){c.InitZOrder(le);const p=V.toJsonText(Ce.formation),u=V.toJsonText(be.formation),k=pe.toJsonText(c),f=new window.JSZip;f.file(He,p),f.file(Re,u),f.file(Fe,k);const y=await f.generateAsync({type:"blob"}),T="gameConfig.zip",H=URL.createObjectURL(y),B=document.createElement("a");B.href=H,B.download=T,B.click(),URL.revokeObjectURL(H),console.log("saved!")}let De=null,oe=null,ae=null;async function Xe(p){const u=document.createElement("input");return u.type="file",u.accept=".zip",u.addEventListener("cancel",()=>(console.log("Cancelled."),Z.Cancel)),u.addEventListener("change",async()=>{if(u.files.length==1){console.log("File selected: ",u.files[0].name);const f=await u.files[0].arrayBuffer(),T=await new window.JSZip().loadAsync(f);async function H(S){const _=T.file(S);if(_){const N=await _.async("string"),x=JSON.parse(N);return console.log(x),x}}{const S=await H(Fe);S&&(De=S)}{const S=await H(He);S&&(oe=S)}{const S=await H(Re);S&&(ae=S)}const B=De!==null&&oe!==null&&ae!==null?Z.Success:Z.Fail;p(B)}}),u.click(),Z.Unknown}const le=new M;e&&(le.AddDialogs(),le.AssignIndexies(),c.InitZOrder(le)),o.hideFullScreenCss(a);function Be(p){const u=new ve,k=new Le,f=p.formation;f.uiInfo.name,f.uiInfo.name;let y=0;for(const T of f.items){const H=p.scsList[y];y++;const B=Math.ceil(H.stdScore),S=new Ve(T.ch.id,T.ch.name,T.isEmpty,B);S.allAvailable=H.allAvailable,k.Add(S)}return u.Add(k),u.debug(),u}function Pe(){const p=new pt,u=Be(Ce),k=Be(be),f=new Te;f.setScoreItems(u.groupRows[0].columns),f.boost=100;const y=new Te;y.setScoreItems(k.groupRows[0].columns),y.boost=100;const T=new dt;T.setPlayer(f),T.setEnemy(y),p.setPair(Ie.HiLv,T),p.calcCombatScore();for(const[H,B]of p.combatPairs){const S=B.judge(P.None);console.log(`judge=[${S}]`)}return p}}class gt{constructor(){this.intervalMs=1e3,this.isOneShot=!0,this.intervalTimer=null}start(e,t){this.intervalTimer!==null&&this.clear(),this.intervalTimer=setTimeout(async()=>{t!==void 0&&(await t(e)||this.clear()),this.isOneShot===!0&&this.clear()},this.intervalMs)}clear(){clearTimeout(this.intervalTimer),this.intervalTimer=null}}class Ue{constructor(){this.key="",this.text="",this.comment=""}}class yt{constructor(){this.itemList=new Array,this.uiInfo=new K,this.parentName="",this.saveTimer=null,this.onSave=async e=>(console.log(`${e.parentName}`),!1)}init(){for(let e=0;e<10;e++)this.itemList.push(new Ue)}async startAutoSave(){this.saveTimer=new gt,this.saveTimer.intervalMs=1e3,this.saveTimer.isOneShot=!1,this.saveTimer.start(this,this.onSave)}setRow(e,t,s){const n=new Ne;n.placeholder=e.key===""?"キー":e.key,s.getCell(0,t).typeInfo.setInput(n),s.getCell(0,t).className=this.itemKeyCssClassName(),s.getCell(0,t).typeInfo.using.itemId=t;const i=new Ne;i.placeholder=e.text===""?"文字列":e.text,s.getCell(1,t).typeInfo.setInput(i),s.getCell(1,t).className=this.itemTextCssClassName(),s.getCell(1,t).typeInfo.using.itemId=t;const o=new Ne;o.placeholder=e.comment===""?"コメント":e.comment,s.getCell(2,t).typeInfo.setInput(o),s.getCell(2,t).className=this.itemCommentCssClassName(),s.getCell(2,t).typeInfo.using.itemId=t,s.getCell(3,t).typeInfo.setButton("削除"),s.getCell(3,t).className=this.itemDeleteCssClassName(),s.getCell(3,t).typeInfo.using.itemId=t}toHTML(){if(!this.itemList)return"";const e=new ue;if(this.itemList.length>=1)e.makeDim(4,this.itemList.length),this.itemList.forEach((i,o)=>{this.setRow(i,o,e)}),e.makeRowTemplate(),this.table=e;else{e.makeDim(4,1);const i=new Ue;this.setRow(i,0,e),e.makeRowTemplate(),e.clearRows(),this.table=e}const t=this.tableCssClassName(),s=this.table.ToScrollHTML(t,"");this.htmlMaker=new D;const n=new F;return n.props.name="",n.props.id="0",n.props.className=t,n.props.option.setTable(s),n.props.option.onSelect=async i=>{switch(console.log(`classify = ${i.classify} targetId = ${i.targetId}`),i.classify){case this.itemKeyCssClassName():break;case this.itemTextCssClassName():break;case this.itemCommentCssClassName():break;case this.itemDeleteCssClassName():const o=new se;o.setParent(this.dlgCssClassName());let a=w.None;switch(o.setYesNo(),a=await o.showWait(`${i.targetId} を削除しますか？`),a){case w.Yes:break;case w.No:return;case w.Cancel:return}const l=parseInt(i.targetId);l>=1&&(e.deleteRow(l),e.redimAllRows(),this.itemList.splice(l-this.table.firstRowIndex,1));break}},this.htmlMaker.add(n),this.htmlMaker.ToHTML()}createSummaryBox(e,t,s){this.parentName=t;const n=this.htmlMaker.MakeDefaultButtonsHTML(`
...
${this.htmlMaker.MakeDefaultButtonsCss()}
`.trim(),document.head.appendChild(s);const n=document.getElementById(this.parentName);this.htmlMaker.copyCssToInlineStyle(`.${this.parentName}`,n)}}async function Ct(m){if(!m.isWebRunning)return;const e=m.isWebRunning;async function t(){const n=new yt;n.init(),n.uiInfo.name="ResourceEdit",n.uiInfo.left="110",n.uiInfo.top="10";const i=n.uiInfo.name,o=await n.toHTML();if(e){const a=document.createElement("div");a.id=n.uiInfo.name,a.className=n.uiInfo.name,document.body.appendChild(a);const r=n.createSummaryBox("文字列リソース",i,o);n.addEventHandlers(r),n.addItemEventHandlers(),r.show()}return n}(await t()).startAutoSave()}const $=new Y,Ge=$.isWebRunning;Ge?($.parseURLParams(),$.currentUserHome===""&&$.setUser($.user1Home)):$.setUser($.user2Home);$.currentUserHome;if($.resMode===""){const m=new se;if(Ge){const e=$.setBrowserTitle();await m.showWait(`「${e} 」モードで起動します`)}}$.setPath();$.statusJsonPath;$.zipPrefix;$.chListFile;$.chStatusListFile;switch($.admin){case!0:await Ze($);break;case!1:$.resMode===""?await ft($):await Ct($);break}
