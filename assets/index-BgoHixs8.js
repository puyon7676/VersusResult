...
width: 100%;
        ...
        `.trim(),document.head.appendChild(t)}enableEvents(e){return this.ctlElem===null?!1:(this.htmlMaker.enableTableEvents(this.ctlElem.className),this.onSelect=e,!0)}disableEvents(){return this.ctlElem===null?!1:(this.htmlMaker.disableTableEvents(this.ctlElem.className),!0)}setVisible(e){return this.ctlElem===null?!1:(this.ctlElem.hidden=!e,!0)}dispose(){super.dispose(),this.ctlElem!==null&&(this.ctlElem.parentElement?.removeChild(this.ctlElem),this.ctlElem.remove()),this.disableEvents(),this.ctlElem=null}tblCssName(){return`${this.ctlName}-tbl`}}class _t{constructor(){this.key="",this.text=""}}class Zt extends at{constructor(){super(...arguments),this.callerName="",this.htmlMaker=new v,this.table=new W,this.ctlElem=null,this.headerElem=null,this.footerElem=null}setGridtems(e){this.items=e}setListener(e,t,s,i="",n=""){if(this.items===void 0)return!1;this.dispose(),this.callerName=s,this.ctlName=e;const o=this.items.length,l=new W;l.makeDim(2,o);let a=0;for(const B of this.items)l.getCell(0,a).typeInfo.setLabel(`${B.text}`,!1),l.getCell(0,a).className=this.keyCssName(),l.getCell(0,a).typeInfo.using.itemId=a,l.getCell(1,a).typeInfo=B.value.items[0].typeInfo,l.getCell(1,a).className=this.valueCssName(),l.getCell(1,a).typeInfo.using.itemId=a,a++;const r=this.firstAction(e,t);if(r===null)return!1;const c=l.ToScrollHTML(e,this.tblCssName()),h=new V;h.props.name="",h.props.id=this.tblCssName(),h.props.className=this.tblCssName(),h.props.option.setTable(c),h.props.option.onSelect=async B=>{if(this.onSelect!==void 0){const A=new Ie;A.callerName=B.classify,A.result=B.selectedValue,await this.onSelect(A)}},this.htmlMaker=new v,this.htmlMaker.add(h);const u=this.htmlMaker.ToHTML();let d=document.createElement("dialog");d.className=e,d.innerHTML=u,this.ctlElem=d;let f=d.children[0];const $=this.divCssName();f.className=$,f.id=$;let k=f.children[0];return k.className=`${this.tblCssName()}`,k.id=`${this.tblCssName()}`,i!==""&&(this.headerElem=document.createElement("div"),this.headerElem.className=i,this.headerElem.id=i,d.appendChild(this.headerElem)),r.appendChild(this.ctlElem),n!==""&&(this.footerElem=document.createElement("div"),this.footerElem.className=n,this.footerElem.id=n,d.appendChild(this.footerElem)),this.table=l,!0}show(e,t){this.ctlElem!==null&&(this.ctlElem.style.left=`${e}px`,this.ctlElem.style.top=`${t}px`,this.ctlElem.show())}setFontConfig(e="0.9"){this.table.setFontConfig(e)}applyCss(){const e=`${this.getBaseElem().className}-flgrd-style`;if(document.getElementById(e))return;const t=document.createElement("style");t.id=e,t.textContent=`
${this.getOuterCssText()}
...
width: 100%;
        ...
        `.trim(),document.head.appendChild(t)}enableEvents(e){return this.ctlElem===null?!1:(this.htmlMaker.enableTableEvents(this.ctlElem.className),this.onSelect=e,!0)}disableEvents(){return this.ctlElem===null?!1:(this.htmlMaker.disableTableEvents(this.ctlElem.className),!0)}setVisible(e){return this.ctlElem===null?!1:(this.ctlElem.hidden=!e,!0)}dispose(){super.dispose(),this.headerElem!==null&&(this.headerElem.parentElement?.removeChild(this.headerElem),this.headerElem.remove()),this.footerElem!==null&&(this.footerElem.parentElement?.removeChild(this.footerElem),this.footerElem.remove()),this.ctlElem!==null&&(this.ctlElem.parentElement?.removeChild(this.ctlElem),this.ctlElem.remove()),this.disableEvents(),this.ctlElem=null}divCssName(){return`${this.ctlName}-div`}tblCssName(){return`${this.ctlName}-tbl`}keyCssName(){return`${this.ctlName}-key`}valueCssName(){return`${this.ctlName}-value`}}class es{constructor(){this.itemType=x.Img,this.itemId=-1,this.label="",this.innerHTML=""}}class Et{constructor(){this.toolTip="",this.using=new es}setButton(e){this.using.itemType=x.Btn,this.using.label=e}setLabel(e,t){this.using.itemType=t?x.Label:x.LabelRO,this.using.label=e}setCombo(e){this.using.itemType=x.Combo,this.using.combo=e}setInput(e){this.using.itemType=x.Input,this.using.input=e}setImg(e){this.using.itemType=x.Img,this.using.img=e}setPlain(e){this.using.itemType=x.Plain,this.using.innerHTML=e}setTable(e){this.using.itemType=x.Table,this.using.innerHTML=e}ToButtonHTML(e){const t=this.using.itemId>=0?` item-id="${this.using.itemId}"`:"";return`
...
`.trim(),document.head.appendChild(l);const a=document.getElementById(this.parentName);this.htmlMaker.copyCssToInlineStyle(`.${this.parentName}`,a)}}const we={Unknown:"Unknown",Success:"Success",Fail:"Fail",Cancel:"Cancel"};async function Tt(m){const e=m.isWebRunning,t=m.currentUserHome,s=m.chStatusListFile,i=m.chListFile,n=new Kt,o=new v,l="splash-screen";o.initFullScreen(l,"エボナ データベース初期化中..."),o.applyFullScreenCss(l);const a=o.getFullScreenMsgElem(l),r=a!==null?a.innerHTML:"";if(t===m.user1Home){const p=[{ns:H.CnsRed,nsName:"赤属性"},{ns:H.CnsBlue,nsName:"青属性"},{ns:H.CnsGreen,nsName:"緑属性"},{ns:H.CnsYellow,nsName:"黄属性"},{ns:H.CnsViolet,nsName:"紫属性"}],M=await new Te().LoadList(i);for(const C of p)a!==null&&(a.innerHTML=`${r} ${C.nsName}`),await n.setupNs(C.ns,m,M)}if(t===m.user2Home){const p=[{ns:H.CnsBlue,nsName:"藍属性"},{ns:H.CnsRed,nsName:"紅属性"},{ns:H.CnsGreen,nsName:"翠属性"},{ns:H.CnsYellow,nsName:"黄属性"},{ns:H.CnsWhite,nsName:"天属性"},{ns:H.CnsBlack,nsName:"冥属性"}],M=await new Te().LoadList(i);for(const C of p)a!==null&&(a.innerHTML=`${r} ${C.nsName}`),await n.setupNs(C.ns,m,M)}a!==null&&(a.innerHTML="UI 初期化中 ...");const c=new Ge,h=new qe;await h.loadDB(s);const u=await h.loadAuxScoreSet(m.gameType);async function d(){async function p(){return await new Te().LoadList(i)}const g=await p();g.uiInfo.name="charListArea",g.uiInfo.left="300",g.uiInfo.top="100";const M=g.uiInfo.name,C=await g.toHTML(n);if(e){const y="キャラ選択",S=g.createSelectorBox(y,M,C);g.addEventHandlers(S),g.addItemEventHandlers(),g.enableLazyImages(n),S.show();const T=new Y;T.setAsDlg(S,y),c.add(T)}return g}const f=await d();async function $(){const p=new rs;await p.load(),p.uiInfo.name="CharSummary",p.uiInfo.left="400",p.uiInfo.top="100";const g=p.uiInfo.name,M=await p.toHTML();if(e){const C="キャラ一覧",y=p.createSummaryBox(C,g,M);await p.updateCharInfos(n),p.addEventHandlers(y,async T=>{T.selectCh=f.selectedCh;const P=await n.getImageUrlBy(f.selectedCh.iconFileName,f.selectedCh.ns);if(P===null)T.selectedImg="",T.cancel=!0;else{T.selectedImg=P,T.cancel=!1;const L=await h.getStatus(f.selectedCh.idAsText);console.log(L),T.scoreSet=L,T.scoreSet.items=T.scoreSet.items.concat(u.items)}console.log(`selected ch = ${T.selectCh.name}`)}),p.addItemEventHandlers(),p.enableLazyImages(n),y.show();const S=new Y;S.setAsDlg(y,C),c.add(S)}return p}const k=await $();async function B(p,g,M,C){const y=new hs;p===A&&(y.InitForPlayer(k),y.autoForm=!0,y.editFormEnable=!1,y.saveEnable=!1,y.imgLoader=n),p===J&&(y.InitForEnemy(h),y.SetAuxScoreSet(u),y.autoForm=!1,y.editFormEnable=!0,y.saveEnable=!0),y.formation.uiInfo.name=p,y.formation.uiInfo.left=`${g}`,y.formation.uiInfo.top=`${M}`;const S=y.formation.uiInfo.name,T=await y.toHTML(S),P=p===J?y.toGridHTML():"";if(e){const L=y.createFormationBox(C,S,T,P);y.addEventHandlers(L),y.addItemEventkHandlers(async E=>{let D="",We="";E.selectedImg===""?(D=f.selectedCh.iconFileName,We=f.selectedCh.ns,E.selectCh=f.selectedCh):(D=E.item.ch.iconFileName,We=E.item.ch.ns,E.selectCh=E.item.ch);const bt=await n.getImageUrlBy(D,We);bt!==null&&(E.selectedImg=bt,E.item.isEmpty=!1,Ct(E.uiName,E.item,E.selectCh))},async E=>{const D=await n.getImageUrlBy(E.selectedImg,f.selectedCh.ns);D!==null&&(E.selectedImg=D,E.item.isEmpty=!0,Ct(E.uiName,E.item,E.selectCh))},async E=>{console.log(`selected ch = ${E.item.ch.name}`);const D=new At;D.selectCh=E.item.ch,D.selectedImg=E.selectedImg,D.scoreSet=E.scoreSet,k.charStock(D)},async E=>{const D=yt();console.log(D),await D.replaceJudge(n)}),y.enableLazyImages(n),L.show();const ye=new Y;ye.setAsDlg(L,C),c.add(ye)}return y}const A="playerForm",J="enemyForm",Ee=await B(A,100,100,"自編成"),xe=await B(J,100,200,"敵編成");async function Pt(p,g,M,C){q.uiInfo.name=p,q.uiInfo.left=`${g}`,q.uiInfo.top=`${M}`;const y=await q.toHTML("combatTable",n),S=q.createCombatBox(C,p,y);q.enableLazyImages(n),await q.replaceJudge(n),S.show();const T=new Y;T.setAsDlg(S,C),c.add(T)}const q=yt();await Pt("combatForm",120,300,"対戦予想");const ht="保存";{const p=new Y;p.setAsMenu(ht),c.add(p)}const mt="復帰";{const p=new Y;p.setAsMenu(mt),c.add(p)}let Me=null;const Ot=await c.toHTML("dockForm",n);if(e){const p=c.createDockBox("dockForm",Ot);c.addItemClickHandlers(async g=>{c.stdApplyAction(g)!==!1&&g.item.isMenuType&&(g.item.toolTip===ht&&await Ut(),g.item.toolTip===mt&&await zt(async C=>{if(console.log(`[loadedResult] ${C}`),C!==we.Success)return;const y=X.fromJsonInst(ve),S=X.fromJsonInst(Le);ve=null,Le=null,await Ee.Setup(y,n),await xe.Setup(S,n)}))}),c.enableLazyImages(n),p.show(),Me=p}const dt="playerForm.json",ut="enemyForm.json",ft="dockForm.json";async function Ut(){c.InitZOrder(_);const p=X.toJsonText(Ee.formation,Ee.scsList),g=X.toJsonText(xe.formation,xe.scsList),M=Ve.toJsonText(c),C=new window.JSZip;C.file(dt,p),C.file(ut,g),C.file(ft,M);const y=await C.generateAsync({type:"blob"}),S="gameConfig.zip",T=URL.createObjectURL(y),P=document.createElement("a");P.href=T,P.download=S,P.click(),URL.revokeObjectURL(T),console.log("saved!")}let pt=null,ve=null,Le=null;async function zt(p){const g=document.createElement("input");return g.type="file",g.accept=".zip",g.addEventListener("cancel",()=>(console.log("Cancelled."),we.Cancel)),g.addEventListener("change",async()=>{if(g.files.length==1){console.log("File selected: ",g.files[0].name);const C=await g.files[0].arrayBuffer(),S=await new window.JSZip().loadAsync(C);async function T(L){const ye=S.file(L);if(ye){const E=await ye.async("string"),D=JSON.parse(E);return console.log(D),D}}{const L=await T(ft);L&&(pt=L)}{const L=await T(dt);L&&(ve=L)}{const L=await T(ut);L&&(Le=L)}const P=pt!==null&&ve!==null&&Le!==null?we.Success:we.Fail;p(P)}}),g.click(),we.Unknown}const _=new b;e&&(_.AddDialogs(),_.AssignIndexies(),await _.LoadAllSetting(),await _.loadSetting(Me),c.InitZOrder(_),await _.ForEachAsync(p=>{const g=j.FindDialogParent(p);return g!==null&&(m.isLocal||p==="charListArea"?g.hidden=!1:g.hidden=!0),!0}),Me!==null&&(Me.parentElement.hidden=!m.isLocal)),o.hideFullScreenCss(l);function gt(p){const g=new ct,M=new rt,C=p.formation;C.uiInfo.name,C.uiInfo.name,p.scsList===void 0&&console.log(`[${p.formation.uiInfo.name}] 対戦予想結果の作成は工事中です`);let y=0;for(const S of C.items){if(p.scsList===void 0)continue;const T=p.scsList[y];y++;const P=Math.ceil(T.stdScore),L=new Ht(S.ch.id,S.ch.name,S.isEmpty,P);L.allAvailable=T.allAvailable,M.Add(L)}return g.Add(M),g.debug(),g}function yt(){const p=new us,g=gt(Ee),M=gt(xe),C=new it;C.setScoreItems(g.groupRows[0].columns),C.boost=100;const y=new it;y.setScoreItems(M.groupRows[0].columns),y.boost=100;const S=new ds;S.setPlayer(C),S.setEnemy(y),p.setPair(It.HiLv,S),p.calcCombatScore();for(const[T,P]of p.combatPairs){const L=P.judge(G.None);console.log(`judge=[${L}]`)}return p}async function Ct(p,g,M){const C=It.HiLv;q.combatPairs.get(C),p===A&&await q.replaceChar(C,Q.Player,g,M,n),p===J&&await q.replaceChar(C,Q.Enemy,g,M,n),await q.replaceJudge(n)}}async function fs(m){if(!m.isWebRunning)return;const e=m.isWebRunning;console.log(`mode=${m.edit}`);const t=new Ge;async function s(){const u=ae.Resource,d=new Nt;d.init(),await d.load(u,z.none),d.uiInfo.name="ResourceEdit",d.uiInfo.left="110",d.uiInfo.top="10";const f=d.uiInfo.name,$=await d.toHTML(d.makeResEditItems(),d.makeResLog);if(e){const k=document.createElement("div");k.id=d.uiInfo.name,k.className=d.uiInfo.name,document.body.appendChild(k);const B="文字列リソース",A=d.createEditorBox(B,f,$);d.addEventHandlers(A),d.addItemEventHandlers(),A.show(),d.enableResize();const J=new Y;J.setAsDlg(A,B),t.add(J)}return d}async function i(){const u=ae.ScoreUI,d=new Nt;d.init(),await d.load(u,m.edit),d.uiInfo.name="ScoreEdit",d.uiInfo.left="110",d.uiInfo.top="100";const f=d.uiInfo.name,$=await d.toHTML(d.makeScoreEditItems(),d.makeScoreLog);if(e){const k=document.createElement("div");k.id=d.uiInfo.name,k.className=d.uiInfo.name,document.body.appendChild(k);const B="スコア設定",A=d.createEditorBox(B,f,$);d.addEventHandlers(A),d.addItemEventHandlers(),A.show(),d.enableResize();const J=new Y;J.setAsDlg(A,B),t.add(J)}return d}(await s()).startAutoSave(),(await i()).startAutoSave();const l=document.createElement("div"),a="dockEdit";l.id=a,l.className=a,document.body.appendChild(l);let r=null;const c=await t.toHTML(a,null);if(e){const u=t.createDockBox(a,c);t.addItemClickHandlers(async d=>{t.stdApplyAction(d)}),u.show(),r=u}const h=new b;e&&(h.AddDialogs(),h.AssignIndexies(),await h.LoadAllSetting(),await h.loadSetting(r),t.InitZOrder(h),await h.ForEachAsync(u=>{const d=j.FindDialogParent(u);return d!==null&&(m.isLocal||u==="charListArea"?d.hidden=!1:d.hidden=!0),!0}),r!==null&&(r.parentElement.hidden=!m.isLocal))}const ce={None:"None",Full:"Full",Limit:"Limit",NotRun:"NotRun"},R=new $e,Ft=R.isWebRunning;Ft?(gs(),R.parseURLParams(),R.currentUserHome===""&&R.setUser(R.user1Home)):R.setUser(R.user2Home);const je=window.EVONA_CONFIG.isLocal;R.setPath();O.encodeEnable=!je;R.setImageHome(je);const ps=ys(je);R.currentUserHome;R.statusJsonPath;R.zipPrefix;R.chListFile;R.chStatusListFile;switch(R.admin){case!0:await jt(R);break;case!1:if(Ft)if(R.edit===z.none){let m=!1,e=!1;const t=new ue;t.CheckVisible=!0,t.CheckText="位置情報を初期化",t.SecretEnable=!0;const s=R.setBrowserTitle();await t.showWait(`「${s} 」モードで起動します`)===N.Secret&&(e=!0),m=t.Checked,console.log(`secretMode=[${e}]`),console.log(`cleanMode=[${m}]`);let n=ce.None;if(je)n=ce.Full;else if(n=ce.NotRun,e&&ps){let o="";t.CheckVisible=!1,t.SecretEnable=!1,t.AuthVisible=!0,t.onAuthChecking=u=>(o=u,!0),await t.showWait("認証コードを入力してください");const l=new Date,a=("0"+(l.getMonth()+1)).slice(-2),r=("0"+l.getDate()).slice(-2),h=Math.random()>.5?`${a}${r}`:`${r}${a}`;o===h?(console.log("auth success"),n=ce.Limit):console.log("auth fail")}switch(m&&(await(await Jt()).clear(),await(await oe()).clear()),n){case ce.Full:window.EVONA_CONFIG.demo=!1,await Tt(R);break;case ce.Limit:window.EVONA_CONFIG.demo=!0,await Tt(R);break}}else R.setBrowserTitle(),await fs(R);break}function gs(){const m=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"||window.location.hostname.startsWith("192.168."),e=m?"./jszip_dist/jszip.min.js":"https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js",t=m?"./jsstore_dist/jsstore.min.js":"https://cdn.jsdelivr.net/npm/jsstore/dist/jsstore.min.js";window.EVONA_CONFIG={isLocal:m,workerFile:"./jsstore_dist/jsstore.worker.min.js",demo:!m};function s(i,n=!1){const o=document.createElement("script");o.src=i,n&&(o.type="module"),o.async=!1,document.head.appendChild(o)}s(e),s(t)}function ys(m){if(m)return!0;const e=document.referrer;console.log(`${e}`);const t="evona_auth",s="granted";return sessionStorage.getItem(t)===s?!0:e.includes("qiita.com")?(sessionStorage.setItem(t,s),!0):!1}
